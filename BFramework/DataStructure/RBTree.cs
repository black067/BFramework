...
namespace BFramework.DataStructure
{
    ...
    public class RedBlackTree<T>
    {
        ...
        {
            return Contain(value, out RedBlackTreeNode<T> node);
        }
        ...
        public bool Contain(T value, out RedBlackTreeNode<T> newNode)
        {
            ...
            while (node != null)
            {
                int comparer = _comparer.Compare(value, node.Data);
                if (comparer > 0)
                {
        ...
        public void Add(T value)
        {
            ...
            {
                // 根节点是黑色的  
                _root = new RedBlackTreeNode<T>(value, BLACK);
            }
            else
            {
                // 新插入节点是红色的  
        ...
        private void Insert1(RedBlackTreeNode<T> newNode, T value)
        {
            ...
            int comparer1 = _comparer.Compare(value, parent.Data);
            if (comparer1 > 0)
            {
        ...
        public T Min
        {
            get
            {
                if (_root != null)
                {
                    RedBlackTreeNode<T> node = GetMinNode(_root);
                    return node.Data;
                }
                else
                {
        ...
        private void RemoveFixUp(RedBlackTreeNode<T> node, RedBlackTreeNode<T> parent)
        {
            ...
            while ((node == null || IsBlack(node)) && (node != _root))
            {
